...
def save_processed_data(X_train, X_test, y_train, y_test, output_path):
    ...
    y_train.to_csv(os.path.join(processed_dir, 'y_train.csv'), index=False, header=['target'])
    y_test.to_csv(os.path.join(processed_dir, 'y_test.csv'), index=False, header=['target'])
