...
from services.predict.main import DummyHousingModel, prepare_features, load_model
...
class TestPredictServiceHelpers:
    ...
    def test_load_model_success(self, test_settings):
        ...
        with patch('config.settings.settings', test_settings):
            ...
            with patch('services.predict.main.model', None):
                result = load_model()
    ...
    def test_load_model_failure(self, test_settings):
        ...
        with patch('config.settings.settings', test_settings):
            with patch('services.predict.main.DummyHousingModel') as mock_model:
...
class TestPredictServiceEndpoints:
    ...
    async def test_health_endpoint_model_loaded(self, predict_client):
        ...
        with patch('services.predict.main.model', Mock()):
    ...
    async def test_predict_endpoint_success(self, predict_client):
        ...
        with patch('services.predict.main.model', mock_model):
            request_data = {
                "longitude": -122.23,
                "latitude": 37.88,
                "housing_median_age": 41.0,
    ...
    async def test_reload_model_endpoint_success(self, predict_client):
        ...
        with patch('services.predict.main.load_model', return_value=True):
    ...
    async def test_reload_model_endpoint_failure(self, predict_client):
        ...
        with patch('services.predict.main.load_model', return_value=False):
            response = await predict_client.post("/model/reload")
