...
class TestAPIHealthEndpoints:
    ...
    async def test_readiness_endpoint_success(self, api_client):
        ...
        with patch('services.api.main.http_client') as mock_client:
...
class TestAPIPredictionEndpoint:
    ...
    async def test_prediction_success(self, api_client, sample_prediction_request):
        ...
        with patch('services.api.main.call_predict_service') as mock_call:
    ...
    @pytest.mark.asyncio
    async def test_prediction_predict_service_error(self, api_client, sample_prediction_request):
        """Test prediction when predict service returns error"""
        with patch('services.api.main.call_predict_service') as mock_call:
            from fastapi import HTTPException
            mock_call.side_effect = HTTPException(status_code=503, detail="Service unavailable")
    ...
    @pytest.mark.asyncio
    async def test_prediction_internal_error(self, api_client, sample_prediction_request):
...
class TestCallPredictService:
    ...
    async def test_successful_call(self, test_settings, mock_predict_service_response):
        ...
        with patch('config.settings.settings', test_settings):
            from services.api.main import call_predict_service
