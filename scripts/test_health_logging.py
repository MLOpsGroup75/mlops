...
def test_endpoint(url: str, endpoint_name: str):
    ...
    try:
        ...
        print(
            f"✓ {endpoint_name}: Status {response.status_code}, Duration: {duration:.3f}s"
        )
        return True
...
def test_health_logging_feature():
    ...
    print(
        f"Expected behavior: {'Logging ENABLED' if settings.log_all_endpoints else 'Logging DISABLED'} for health endpoints"
    )
    print()
    ...
    print(
        f"   - Health endpoints (/health, /readiness, /metrics) should {'APPEAR' if settings.log_all_endpoints else 'NOT APPEAR'} in logs"
    )
...
def show_configuration_options():
    ...
    print("  1. Set environment variable: LOG_ALL_ENDPOINTSS=true")
    print("  2. Or modify config/settings.py: log_all_endpoints = True")
    print()
    print("To DISABLE health endpoint logging (default):")
    print("  1. Set environment variable: LOG_ALL_ENDPOINTSS=false")
