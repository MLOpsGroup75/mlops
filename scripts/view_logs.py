...
def show_summary(db_path):
    ...
    try:
        ...
        # Count by logger
        cursor.execute(
            "SELECT logger_name, COUNT(*) FROM logs GROUP BY logger_name ORDER BY COUNT(*) DESC"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--all", action="store_true", help="Show all entries (no limit)"
    )
    parser.add_argument("--service", type=str, help="Filter by service name")
    parser.add_argument(
        "--level", type=str, help="Filter by log level (INFO, ERROR, WARNING, etc.)"
    )
    parser.add_argument("--request-id", type=str, help="Filter by specific request ID")
    parser.add_argument(
