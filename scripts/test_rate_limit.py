...
def test_rate_limiting():
    ...
    print(
        f"Rate limit: {settings.rate_limit_requests} requests per {settings.rate_limit_window} seconds"
    )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    print(
                        f"   Retry After: {error_data.get('retry_after', 'Unknown')} seconds"
                    )
                except:
...
def test_rate_limit_recovery():
    ...
    print(
        f"Waiting {settings.rate_limit_window + 1} seconds for rate limit window to reset..."
    )
