...
async def startup_event():
    ...
    if success:
        ...
        logger.error(
            "Failed to start predict service - model loading failed", service="predict"
        )
...
async def predict_housing_price(
    request: Request, prediction_request: InternalPredictionRequest
) -> InternalPredictionResponse:
    ...
    logger.info(
        "Processing prediction request", request_id=request_id, service="predict"
    )
...
async def health_check():
    ...
    return HealthResponse(
        status=model_status, timestamp=datetime.utcnow().isoformat(), service="predict"
    )
