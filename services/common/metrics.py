...
class MetricsCollector:
    ...
    def __init__(self, service_name: str = "mlops"):
        ...
        self.service_up = Gauge("service_up", "Service availability", ["service"])
    ...
    def record_prediction(self, duration: float, status: str = "success"):
        ...
        self.prediction_requests_total.labels(
            service=self.service_name, status=status
        ).inc()

        self.prediction_duration_seconds.labels(service=self.service_name).observe(
            duration
        )

    def record_rate_limit_exceeded(self):
        """Record a rate limit exceeded event"""
        self.rate_limit_exceeded_total.labels(service=self.service_name).inc()
