...
def main():
    ...
    try:
        ...
        if missing_files:
            ...
            logger.info(
                "You can process the data using the preprocessing scripts in data/src/"
            )
            return
        ...
        if "DATABRICKS_HOST" in os.environ and "DATABRICKS_TOKEN" in os.environ:
            logger.info(
                "Databricks credentials found, using Databricks SaaS for MLflow"
            )
        else:
            logger.info("No Databricks credentials found, using local MLflow tracking")
            logger.info(
                "To use Databricks SaaS, set DATABRICKS_HOST and DATABRICKS_TOKEN environment variables"
            )
