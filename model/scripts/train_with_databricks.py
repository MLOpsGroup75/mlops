...
def setup_databricks_credentials(host=None, token=None, interactive=True):
    ...
    if not final_host and interactive:
        final_host = input(
            "Enter Databricks host URL (or press Enter to use local MLflow): "
        ).strip()
        if final_host:
...
def run_training_pipeline(use_databricks=True, quick_mode=False):
    ...
    try:
        ...
        tracking_uri = (
            "databricks"
            if use_databricks and os.environ.get("DATABRICKS_TOKEN")
            else "file:./mlruns"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train California housing models with MLflow"
    )

    parser.add_argument("--databricks-host", type=str, help="Databricks workspace URL")
    parser.add_argument("--databricks-token", type=str, help="Databricks access token")
    parser.add_argument(
        "--local-only",
        action="store_true",
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick mode with fewer models for testing"
    )
    parser.add_argument(
        "--non-interactive",
        action="store_true",
    ...
    success = run_training_pipeline(
        use_databricks=use_databricks, quick_mode=args.quick
    )
