...
class BaseTrainer(ABC):
    ...
    def _setup_mlflow(self):
        ...
        try:
            ...
            if self.tracking_uri == "databricks":
                ...
                if "DATABRICKS_HOST" in os.environ and "DATABRICKS_TOKEN" in os.environ:
                    ...
                    if exp_manager.setup_experiment(self.experiment_name):
                        ...
                        current_exp = mlflow.get_experiment_by_name(
                            self.experiment_name
                        )
                        if current_exp is None:
                            # Find the experiment that was actually created
                            candidates = exp_manager.get_experiment_path_candidates(
                                self.experiment_name
                            )
                            for candidate in candidates:
                                ...
                                if exp is not None:
                                    ...
                                    break
                        logger.info(
                            f"✅ Databricks experiment ready: {self.experiment_name}"
                        )
                    else:
            ...
            if not (
                self.tracking_uri == "databricks" and "DATABRICKS_HOST" in os.environ
            ):
                try:
                    ...
                    fallback_name = (
                        f"fallback_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}"
                    )
    ...
    def load_data(
        self, data_dir: str = "data/processed"
    ...
    def cross_validate_model(
        ...
        logger.info(
            f"Cross-validation R² score: {cv_metrics['cv_r2_mean']:.4f} (+/- {cv_metrics['cv_r2_std'] * 2:.4f})"
        )
    ...
    def create_visualizations(
        ...
        if hasattr(self.model, "feature_importances_") and feature_names:
            ...
            plt.xticks(
                range(len(coef)), [feature_names[i] for i in indices], rotation=45
            )
            plt.grid(True, alpha=0.3)
    ...
    def tune_hyperparameters(
        self, X_train: pd.DataFrame, y_train: pd.Series, cv: int = 5, **kwargs
    ) -> Dict[str, Any]:
        """
