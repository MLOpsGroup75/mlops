...
class LinearRegressionTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        experiment_name: str = "california_housing_linear",
    ...
    def _get_param_grid(self, search_type: str = "grid") -> Dict[str, Any]:
        ...
        if self.model_type == "linear":
            ...
            if search_type == "grid":
                return {"model__alpha": [0.01, 0.1, 1.0, 10.0, 100.0, 1000.0]}
            else:
                return {"model__alpha": np.logspace(-3, 3, 100)}

        elif self.model_type == "lasso":
            if search_type == "grid":
                return {"model__alpha": [0.001, 0.01, 0.1, 1.0, 10.0, 100.0]}
            else:
                return {"model__alpha": np.logspace(-4, 2, 100)}
    ...
    def tune_hyperparameters(
        ...
        **kwargs,
        ...
        if self.model_type == "linear":
            ...
            return self.train(
                X_train, y_train, X_train, y_train, use_scaling=use_scaling
            )
