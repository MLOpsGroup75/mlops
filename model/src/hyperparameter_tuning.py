...
class HyperparameterTuner:
    ...
    def optimize_with_optuna(
        ...
        # Create study
        study_name = (
            study_name or f"study_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        if mlflow_logging:
            try:
                mlflow_callback = MLflowCallback(
                    tracking_uri=self.tracking_uri, metric_name="objective_value"
                )
        ...
        results = {
            ...
            "optimization_history": [
                trial.value for trial in self.study.trials if trial.value is not None
            ],
        }
    ...
    def get_linear_regression_param_distributions(
        self, model_type: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        if model_type == "ridge":
            return {"alpha": {"type": "float", "low": 1e-5, "high": 1e3, "log": True}}
        elif model_type == "lasso":
            return {"alpha": {"type": "float", "low": 1e-5, "high": 1e2, "log": True}}
        elif model_type == "elastic":
            return {
                "alpha": {"type": "float", "low": 1e-5, "high": 1e2, "log": True},
    ...
    def get_decision_tree_param_distributions(self, model_type: str) -> Dict[str, Any]:
        ...
        if model_type == "decision_tree":
            return {
                ...
                "ccp_alpha": {"type": "float", "low": 0.0, "high": 0.1},
            }
    ...
    def tune_model_with_optuna(
        ...
        **kwargs,
        ...
        if "linear" in model_type.lower() or model_type in [
            "ridge",
            "lasso",
            "elastic",
        ]:
            param_distributions = self.get_linear_regression_param_distributions(
                model_type
            )
        else:
        ...
        if not param_distributions:
            logger.warning(
                f"No parameter distributions defined for model type: {model_type}"
            )
            return {}
