...
class UnityCatalogModelRegistry:
    ...
    def register_model_safely(self, model_uri: str, model_name: str, 
        ...
        try:
            ...
            logger.info(f"✅ Successfully registered model: {full_model_name}, version: {model_version.version}")
            ...
            if tags and hasattr(model_version, 'version'):
                try:
                    ...
                    logger.info(f"✅ Added tags to model version {model_version.version}")
            ...
            if "legacy workspace model registry is disabled" in error_msg:
                logger.warning("⚠️ Legacy model registry disabled - model logged but not registered")
                ...
                logger.info("💡 Model artifacts are still available in MLflow run")
                logger.info("💡 Check Unity Catalog permissions or ask admin to register")
                ...
                logger.info(f"💡 Try creating catalog '{self.catalog_name}' and schema '{self.schema_name}'")
    ...
    def suggest_unity_catalog_setup(self):
        ...
        logger.info(f"   - Create schema '{self.catalog_name}.{self.schema_name}' (if not exists)")
...
def register_model_with_unity_catalog(model_uri: str, model_name: str, 
...
def check_unity_catalog_permissions(catalog: str = "main", schema: str = "mlops") -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            models = client.search_registered_models(filter_string=f"name='{test_name}'")
            ...
            if "PERMISSION_DENIED" in str(e):
                ...
                logger.info("⚠️ Unity Catalog permissions unclear (catalog/schema might not exist)")
                return False
