...
class MLTrainingPipeline:
    ...
    def load_data(self):
        ...
        X_train, X_test, y_train, y_test = dummy_trainer.load_data(self.config['data_dir'])
        
        logger.info(f"Data loaded: {X_train.shape[0]} train samples, {X_test.shape[0]} test samples")
        return X_train, X_test, y_train, y_test
    
    def train_linear_regression_models(self, X_train, y_train, X_test, y_test) -> List[Dict[str, Any]]:
        ...
        for variant in linear_config['variants']:
            ...
            try:
                ...
                logger.info(f"{variant} training completed in {result['training_time']:.2f} seconds")
    ...
    def train_decision_tree_models(self, X_train, y_train, X_test, y_test) -> List[Dict[str, Any]]:
    ...
    def run_advanced_hyperparameter_tuning(self, X_train, y_train) -> List[Dict[str, Any]]:
    ...
    def compare_and_select_best_model(self) -> Dict[str, Any]:
        ...
        logger.info(f"Best model selected: {best_model_info['model_type']} "
    ...
    def register_best_model(self) -> str:
        ...
        if not self.config['model_registry']['register_best_model']:
            logger.info("ℹ️ Model registration disabled (recommended for Unity Catalog workspaces)")
            logger.info("💡 Model artifacts are available in MLflow runs for manual registration")
        ...
        try:
            ...
            if unity_config.get('enabled', False):
                ...
                if best_run_id:
                    ...
                    if version and not version.startswith('not_registered'):
                        logger.info(f"✅ Model registered in Unity Catalog as version {version}")
                        return version
                    else:
                ...
                logger.info("Using legacy model registry (may fail in Unity Catalog workspaces)")
                ...
                mock_model_info = {
                    ...
                    'metrics': {
                        self.config['model_selection']['primary_metric']: self.best_model_info['performance']
                    }
                }
            ...
            if "legacy workspace model registry is disabled" in error_msg:
                logger.warning("⚠️ Legacy model registry disabled - this is expected in Unity Catalog workspaces")
                logger.info("💡 Model artifacts are logged and available in MLflow runs")
                logger.info("💡 You can manually register models in Unity Catalog through the UI")
                ...
                for result in self.training_results:
                    ...
                    for key, value in result.items():
                        if isinstance(value, dict):
                            serializable_result[key] = {k: float(v) if isinstance(v, (int, float)) else v 
    ...
    def run_complete_pipeline(self):
        ...
        try:
            ...
            if self.config['models_to_train']['linear_regression']['enabled']:
                linear_results = self.train_linear_regression_models(X_train, y_train, X_test, y_test)
            ...
            if self.config['models_to_train']['decision_tree']['enabled']:
                tree_results = self.train_decision_tree_models(X_train, y_train, X_test, y_test)
            ...
            if self.best_model_info:
                logger.info(f"Best model: {self.best_model_info['model_type']} "
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train California housing prediction models')
    
    parser.add_argument('--config', type=str, help='Path to configuration file')
    parser.add_argument('--tracking-uri', type=str, default='databricks', 
                       help='MLflow tracking URI')
    parser.add_argument('--experiment-name', type=str, default='california_housing_experiment',
                       help='MLflow experiment name')
    parser.add_argument('--data-dir', type=str, default='data/processed',
                       help='Directory containing processed data')
    parser.add_argument('--models', nargs='+', 
                       choices=['linear', 'ridge', 'lasso', 'elastic', 'decision_tree', 'random_forest', 'gradient_boosting'],
        ...
        if any(m in args.models for m in ['linear', 'ridge', 'lasso', 'elastic']):
            ...
            if args.models:
                pipeline.config['models_to_train']['linear_regression']['variants'] = [
                    m for m in args.models if m in ['linear', 'ridge', 'lasso', 'elastic']
                ]
        
        if any(m in args.models for m in ['decision_tree', 'random_forest', 'gradient_boosting']):
            pipeline.config['models_to_train']['decision_tree']['enabled'] = True
            if args.models:
                pipeline.config['models_to_train']['decision_tree']['variants'] = [
                    m for m in args.models if m in ['decision_tree', 'random_forest', 'gradient_boosting']
                ]
    
    if args.no_hyperparameter_tuning:
        for model_type in pipeline.config['models_to_train']:
            pipeline.config['models_to_train'][model_type]['hyperparameter_tuning'] = False
