...
class ModelRegistry:
        ...
        try:
            ...
            if self.tracking_uri == "databricks":
                if "DATABRICKS_HOST" in os.environ and "DATABRICKS_TOKEN" in os.environ:
                    ...
                    logger.info("Using Databricks SaaS for MLflow tracking and registry")
                else:
    ...
    def get_best_models_from_experiments(self, experiment_names: List[str], 
    ...
    def compare_models(self, model_infos: List[Dict[str, Any]], 
        ...
        if comparison_metrics is None:
            comparison_metrics = ['test_r2_score', 'test_rmse', 'test_mae', 'cv_r2_mean']
        ...
        for model_info in model_infos:
            ...
            for param_key in ['model_type', 'hyperparameter_optimization', 'search_type']:
        ...
        if comparison_metrics and comparison_metrics[0] in comparison_df.columns:
            ascending = 'rmse' in comparison_metrics[0].lower() or 'mae' in comparison_metrics[0].lower()
            comparison_df = comparison_df.sort_values(comparison_metrics[0], ascending=ascending)
        
        return comparison_df
    
    def register_best_model(self, model_info: Dict[str, Any], 
        ...
        try:
            ...
            try:
                ...
                self.client.create_registered_model(
                    name=model_name,
                    description=description or f"California housing price prediction model"
                )
    ...
    def cleanup_old_models(self, model_name: str, keep_versions: int = 5,
                          exclude_stages: List[str] = None) -> int:
        """
        ...
        try:
            ...
            deletable_versions = [
                ...
                if v.current_stage not in exclude_stages
            ]
