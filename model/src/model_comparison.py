...
class ModelComparator:
    ...
    def compare_training_results(self, training_results: List[Dict[str, Any]],
    ...
    def create_comparison_visualizations(self, save_dir: str = "../../plots/model_comparison") -> Dict[str, str]:
        ...
        if self.comparison_results.empty:
            logger.error("No comparison results available. Run compare_training_results first.")
            return {}
        ...
        if metric_cols:
            ...
            plt.xticks(x + width * (len(metric_cols) - 1) / 2, 
                      [f"{row['model_type']}" for _, row in df.iterrows()], rotation=45)
            plt.legend()
        ...
        if 'rank' in df.columns:
            ...
            if primary_metric:
                for i, (_, row) in enumerate(df.iterrows()):
                    plt.text(row['rank'] + 0.1, i, f"{row[primary_metric]:.4f}", 
        ...
        if 'training_time' in df.columns or 'best_params_count' in df.columns:
            ...
            if complexity_col and metric_cols:
                ...
                plt.scatter(df[complexity_col], df[primary_metric], 
                          alpha=0.7, s=100, c=df['rank'], cmap='viridis_r')
    ...
    def select_best_model(self, training_results: List[Dict[str, Any]],
        ...
        if len(filtered_df) > 1:
            ...
            for _, row in filtered_df.iterrows():
                ...
                max_performance = filtered_df[selection_criteria['primary_metric']].max()
                ...
                if 'best_params_count' in row and not pd.isna(row['best_params_count']):
                    ...
                    simplicity_score = 1 - (complexity / max_complexity) if max_complexity > 0 else 1
        ...
        best_model_info = {
            ...
            'performance': comparison_df.loc[best_model_idx, selection_criteria['primary_metric']],
    ...
    def generate_model_report(self, save_path: str = "model_comparison_report.html") -> str:
        """
        ...
        for metric, description in metric_descriptions.items():
            if metric in self.comparison_results.columns:
                html_content += f"<li><span class='metric'>{metric}:</span> {description}</li>"
    ...
    def save_comparison_results(self, save_path: str = "model_comparison_results.csv") -> str:
        """
